...
export function PollDayPage() {
  ...
  // Calculate summary stats - use API totals first, fallback to booth reduction
  const totalVoters = pollDay.totalVoters || election?.totalVoters || boothTurnouts.reduce((sum, b) => sum + (b.totalVoters || 0), 0);
  const totalVoted = pollDay.totalVoted || boothTurnouts.reduce((sum, b) => sum + (b.voted || 0), 0);
    ...
    return (
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-16">
          ...
          <p className="text-muted-foreground">Please select an election from the sidebar to view poll day data.</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Real-time voter turnout tracking</p>
        </div>
              ...
              <SelectContent>
                ...
                {parts.map((part: any) => (
                  <SelectItem key={part.id} value={part.id}>
                    Part {part.partNumber} - {part.boothName || part.boothName || '-'}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
                  ...
                  <TableBody>
                    {filteredBooths.map((booth) => (
                      <TableRow key={booth.boothId}>
                        ...
                        <TableCell className="text-right">{(booth.totalVoters || 0).toLocaleString()}</TableCell>
                        <TableCell className="text-right">{(booth.voted || 0).toLocaleString()}</TableCell>
                        <TableCell>
                          <div className="flex items-center gap-2 min-w-[120px]">
                            <ProgressBar value={booth.turnout || 0} className="flex-1" />
                            <span className="text-sm font-medium w-12 text-right">
                              {(booth.turnout || 0).toFixed(1)}%
                            </span>
                          </div>
                        </TableCell>
                        <TableCell>
                    ...
                    <TableRow key={agent.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline">
                          Booth {agent.booth?.boothNumber || agent.booth?.partNumber || agent.booth?.partNumber || '-'}
                        </Badge>
                      </TableCell>
                      <TableCell>
